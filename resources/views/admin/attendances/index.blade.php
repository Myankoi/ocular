    ...
    <dialog id="attendance-correction-dialog" class="border-0 bg-transparent p-0 backdrop:bg-slate-950/50" style="position: fixed; inset: 0; width: min(92vw, 30rem); max-width: min(92vw, 30rem); height: fit-content; margin: auto">
        <div class="border border-slate-200 bg-white shadow-2xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-200 bg-ocular-surface px-5 py-4">
                <div><p class="text-[10px] font-black uppercase tracking-[0.18em] text-ocular-orange">Koreksi log</p><h2 class="mt-1 text-base font-black text-ocular-teal">Ubah status absensi</h2><p data-correction-context class="mt-1 text-xs text-ocular-copy/60"></p></div>
                <button type="button" data-correction-close class="grid size-8 place-items-center border border-slate-200 bg-white text-lg leading-none text-ocular-copy hover:border-ocular-teal hover:text-ocular-teal" aria-label="Tutup koreksi">×</button>
            </div>
            <form id="attendance-correction-form" method="POST" action="#" class="space-y-5 p-5">
                @csrf
                @method('PATCH')
                <p class="text-sm text-ocular-copy">Koreksi untuk <strong data-correction-student class="text-ocular-teal"></strong>.</p>
                ...
                <label class="block text-xs font-bold uppercase tracking-wider text-ocular-teal">Catatan koreksi<textarea name="notes" data-correction-notes rows="3" placeholder="Alasan atau catatan tambahan..." class="mt-2 w-full border border-slate-300 bg-white px-3 py-3 text-sm focus:border-ocular-teal focus:outline-none focus:ring-2 focus:ring-ocular-teal/20"></textarea></label>
                <div class="flex justify-end gap-2 border-t border-slate-100 pt-4"><button type="button" data-correction-cancel class="min-h-10 border border-slate-200 px-3 text-[10px] font-black uppercase tracking-wider text-ocular-copy hover:bg-slate-50">Batal</button><button type="submit" class="min-h-10 bg-ocular-orange px-4 text-[10px] font-black uppercase tracking-wider text-white hover:bg-ocular-orange-dark">Simpan koreksi</button></div>
            </form>
        </div>
    </dialog>
